...
module OmniAuth
  module Strategies
    class Localidp < SAML
      def dev_log(s)
            Decidim::GoteborgLogin::Dbg.dev_log "... localidp_strategy.rb : #{s}"
      end
      ...
      def initialize(app, *args, &block)
        ...
        # option values.
        @localidp_thread = Thread.new do
          @options = OmniAuth::Strategy::Options.new(
            localidp_options.merge(options)
          )
        end
      end
      ...
      def request_phase
        ...
        localidp_thread.join if localidp_thread.alive?
     ...
     option :possible_request_attributes, [
      ...
      # rubocop:disable Metrics/MethodLength
      def localidp_options
        dev_log "localidp_options : 00 : idp_metadata={idp_metadata}"
